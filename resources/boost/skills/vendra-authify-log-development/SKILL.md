---
name: vendra-authify-log-development
description: "Create, modify, review, or test the Vendra Authify Log package in packages/vendra-authify-log. Use for AuthifyLog records, Redis-buffered authentication logging, drain commands, authentication listeners and notifications, policies, Filament resources and filters, configuration, translations, package wiring, and tests."
---
...
- Add or update focused Pest coverage, then run `composer --working-dir=packages/vendra-authify-log test` and `composer --working-dir=packages/vendra-authify-log analyse`.
...
Treat `packages/vendra-authify-log` as the source of authify-log domain behavior and Filament admin UI.

- Use namespace `Misaf\VendraAuthifyLog`.
...
- Do not place authify-log domain code in the host app unless the host app is only integrating the module.
- Keep cross-module dependencies explicit in `composer.json`; do not introduce a dependency without approval.

## Domain Model Standards

Follow the existing `AuthifyLog` patterns for new authify-log entities.
...
Keep every resource that declares a `$cluster`, including its pages, tables, relation managers, and resource-scoped widgets, under `src/Filament/Clusters/Resources/` with the matching `Misaf\VendraAuthifyLog\Filament\Clusters\Resources` namespace and plugin discovery path. Resources without a cluster belong under `src/Filament/Resources/`.
...
- Keep navigation labels and groups configurable through the module `Plugin` and `config/vendra-authify-log.php`. Do not add a `tenant_aware` config value; tenant awareness derives from the bound `TenantResolver`.
...
- Keep Pest architecture tests in `tests/ArchTest.php`: the `php`, `security`, and `laravel` presets, plus an expectation that the module stays tenant-agnostic, e.g. `arch()->expect('Misaf\VendraAuthifyLog')->not->toUse('Misaf\VendraTenant')`.
- Add feature or Livewire tests when changing Filament behavior with meaningful user-visible effects.
- Run module checks from the package when possible: `composer --working-dir=packages/vendra-authify-log test` and `composer --working-dir=packages/vendra-authify-log analyse`.
