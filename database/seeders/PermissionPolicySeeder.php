...
final class PermissionPolicySeeder extends Seeder
{
    ...
    private function seedPermissionPolicies(Tenant $tenant): void
    {
        ...
        $policies = array_column(AuthifyLogPolicyEnum::cases(), 'value');
        ...
        $this->command?->info(sprintf('Successfully seeded %d authify log policy permissions for %s tenant. %d created, %d already existed.', count($policies), $tenant->slug, $createdCount, $existingCount));
    }
}
