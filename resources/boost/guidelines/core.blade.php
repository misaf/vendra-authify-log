...
- Stay decoupled from any concrete user module: resolve the authenticatable model through `Support\AuthifyLogUsers::model()` (`auth.providers.users.model`) and never import `Misaf\VendraUser`. Do not add `misaf/vendra-user` to Composer `require` or `require-dev`.
...
- Keep the complete resource tree under `src/Filament/Clusters/Resources/`, use the matching `Misaf\VendraAuthifyLog\Filament\Clusters\Resources` namespace, and keep plugin discovery aligned. Any future resource without a `$cluster` must instead live under `src/Filament/Resources/`.
...
- Keep Pest architecture tests in `tests/ArchTest.php`: the `php`, `security`, and `laravel` presets plus `arch()->expect('Misaf\VendraAuthifyLog')->not->toUse('Misaf\VendraTenant')` and `->not->toUse('Misaf\VendraUser')`.
